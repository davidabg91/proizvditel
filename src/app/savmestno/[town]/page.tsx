...
export default async function TownGroupPage({
  ...
  const producers = await prisma.producer.findMany({
    where: { published: true, sharedDelivery: true, town: { equals: town } },
  ...
  const region = producers[0].region;
  ...
  return (
    <main className="container-page py-12">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        <h1 className="font-serif text-3xl font-semibold sm:text-4xl">{town}</h1>
        <Badge tone="success">Съвместна доставка</Badge>
      </div>
      <p className="mt-2 max-w-2xl text-muted-foreground">
        {producers.length} стопанства от {town}
